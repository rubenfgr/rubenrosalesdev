...
import { CertificationInputDTO, CertificationUpdateInputDTO } from '../models/certification.model';
import { createCertificationServer, deleteCertificationServer, getCertification, listCertifications, updateCertificationServer } from './certification.server';
...
describe('certification.server', () => {
  ...
  it('createCertificationServer should call createCertification with mapped data', async () => {
    ...
    const input: CertificationInputDTO = {
      name: 'Cert', issuer: 'Org', date: new Date('2024-01-01'), url: undefined, userId: 'u1',
    };
  ...
  it('updateCertificationServer should call updateCertification with mapped data', async () => {
    ...
    const input: CertificationUpdateInputDTO = {
      id: 'id1',
    ...
    expect(
      Object.prototype.hasOwnProperty.call(calledArgs[1], 'url') ? calledArgs[1].url : null
    ).toBe(null);
    expect(result).toBe(mockCert);
  });
