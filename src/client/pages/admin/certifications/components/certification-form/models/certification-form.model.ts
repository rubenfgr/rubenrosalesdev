...
export function getCertificationFieldValidators(t: (key: string) => string) {
  return {
    name: { onChange: z.string().min(2, { message: t("admin.certifications.form.validations.name_required") }) },
    issuer: { onChange: z.string().min(2, { message: t("admin.certifications.form.validations.issuer_required") }) },
    date: { onChange: z.string().min(4, { message: t("admin.certifications.form.validations.date_required") }) },
    url: {
      onChange: z
        .string()
    ...
    userId: { onChange: z.string().min(1, { message: t("admin.certifications.form.validations.user_id_required") }) },
  };
}
